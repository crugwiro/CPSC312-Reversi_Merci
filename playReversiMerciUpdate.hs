...
-- Chooses the next next best coordinates for advanced person vs computer game
choose_next_best_helper:: [(Int, Int)] -> State -> (Int, Int) -> Int -> (Int, Int)
choose_next_best_helper [] _ so_far _ = so_far
choose_next_best_helper (h:t) (State board (int1,int2, char1, char2)) best_coord best_val = 
    if new_int2 > best_val 
    then choose_next_best_helper t (State board (int1,int2, char1, char2)) h new_int2
    else choose_next_best_helper t (State board (int1,int2, char1, char2)) best_coord best_val
    where (State new_board (new_int1,new_int2, new_char1, new_char2)) = (updateBoard h 
                                                                            (updateBoard (choose_best_helper (h:t) (State board (int1,int2, char1, char2)) best_coord best_val) (State board (int1,int2, char1, char2))))
...
choose_best state opp_type = 
    if opp_type == "pvcompH"
        then choose_next_best_helper (list_valid_moves state) state (-1,-1) 0 
    else  
        choose_best_helper (list_valid_moves state) state (-1,-1) 0
